...
// L'API Tidal (contrairement au CDN des segments audio) rate-limite vite
// dès qu'on lui envoie trop de requêtes en même temps. Le téléchargement
// parallèle de plusieurs pistes/batches peut multiplier les appels
// concurrents (getStreamUrl, getTrackContributors, getLyrics, ...), donc on
// borne le nombre de requêtes API Tidal simultanées.
//
// Deux files séparées : les gros volumes de requêtes générées par le
// téléchargement (background) ne doivent jamais pouvoir affamer les
// requêtes interactives (recherche, ouverture d'une playlist/album,
// rafraîchissement de la page), même quand une queue de 300 titres tourne.
...
const interactiveLimiter = makeLimiter(3);
const backgroundLimiter = makeLimiter(3);
...
async function apiGet(session, path, params = {}, urlpre = API_BASE, { background = false } = {}) {
  ...
  const limiter = background ? backgroundLimiter : interactiveLimiter;
  const maxAttempts = 8;

  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    // On ne prend le "slot" que pour la durée du fetch : pendant un backoff
    // 429, on le relâche pour ne pas bloquer les autres requêtes (et surtout
    // pas la file interactive) le temps d'attendre.
...
async function apiGetAllItems(session, path, extraParams = {}) {
  ...
  while (true) {
    ...
    if (!items.length) {
      ...
      throw new Error(`Pagination Tidal interrompue à ${out.length}/${total} éléments.`);
    }
...
function getAlbum(req, res, id, { background = false } = {}) {
  return withAutoRefresh(req, res, (session) =>
    apiGet(session, `albums/${id}`, {}, API_BASE, { background }),
  );
}

function getArtist(req, res, id) {
  return withAutoRefresh(req, res, (session) =>
    apiGet(session, `artists/${id}`),
  );
}

function getTrack(req, res, id) {
  return withAutoRefresh(req, res, (session) =>
    apiGet(session, `tracks/${id}`),
  );
}

function getVideo(req, res, id) {
  return withAutoRefresh(req, res, (session) =>
    apiGet(session, `videos/${id}`),
  );
}

function getPlaylist(req, res, id) {
  return withAutoRefresh(req, res, (session) =>
    apiGet(session, `playlists/${id}`),
  );
}
...
async function getItems(req, res, id, type) {
  ...
  if (type === 'playlist') {
    data = await withAutoRefresh(req, res, (s) =>
      apiGetAllItems(s, `playlists/${id}/items`),
    );
  } else if (type === 'album') {
    data = await withAutoRefresh(req, res, (s) =>
      apiGetAllItems(s, `albums/${id}/items`),
    );
  } else if (type === 'mix') {
    data = await withAutoRefresh(req, res, (s) =>
      apiGetAllItems(s, `mixes/${id}/items`),
    );
  } else {
  ...
  for (const item of data) {
    if (item.type === 'track' && item.item?.streamReady) {
...
async function getByUrl(req, res, rawUrl) {
  ...
  if (!type) {
    throw new Error('Lien Tidal non reconnu.');
  }
...
const mpdParser = new XMLParser({
  ...
  attributeNamePrefix: '',
  isArray: (name) =>
    ['Period', 'AdaptationSet', 'Representation', 'S'].includes(name),
});
    ...
    for (const adaptationSet of period.AdaptationSet || []) {
      ...
      for (const rep of adaptationSet.Representation || []) {
        ...
        if (segTimeline) {
          ...
          for (
            let n = startNumber;
            n < startNumber + segTimeList.length;
            n++
          ) {
            trackUrls.push(
              segTemplate.media.replace('$Number$', String(n)),
            );
          }
        }
...
async function getResolutionList(masterUrl) {
  ...
  for (const chunk of chunks) {
    if (
      !chunk.includes('RESOLUTION=') ||
      !chunk.includes('EXT-X-STREAM-INF:')
    ) {
      continue;
    }
    ...
    const resolution = getSub(chunk, 'RESOLUTION=', 'http')
      .trim()
      .split(',')[0];
  ...
  return withAutoRefresh(req, res, async (session) => {
    ...
    if (resp.manifestMimeType?.includes('vnd.tidal.bt')) {
      const manifest = JSON.parse(
        Buffer.from(resp.manifest, 'base64').toString('utf-8'),
      );
    ...
    throw new Error(
      `Can't get the stream URL, type is ${resp.manifestMimeType}`,
    );
  });
}
...
async function getVideoStreamUrl(req, res, videoId, minHeight = 1080) {
  return withAutoRefresh(req, res, async (session) => {
    ...
    const resp = await apiGet(
      session,
      `videos/${videoId}/playbackinfopostpaywall`,
      params,
    );
    ...
    return (
      variants.find((v) => v.height >= minHeight) ||
...
function getTrackContributors(req, res, id) {
  // Uniquement utilisé par le moteur de téléchargement (tagging) : file
  // "background" pour ne jamais passer devant les requêtes interactives.
  return withAutoRefresh(req, res, (session) =>
    apiGet(session, `tracks/${id}/contributors`, {}, API_BASE, { background: true }),
  );
}

async function getLyrics(req, res, id) {
  try {
    return await withAutoRefresh(req, res, (session) =>
      apiGet(session, `tracks/${id}/lyrics`, {}, LISTEN_BASE, { background: true }),
    );
  } catch {
    return null;
  }
...
function estimateBytesFromDuration(duration, quality) {
  const bitrate =
    ESTIMATED_BITRATES[quality] || ESTIMATED_BITRATES.hifi;
  ...
  return withAutoRefresh(req, res, async (session) => {
    ...
    const tracks = data
      .filter(
        (item) =>
          item.type === 'track' &&
          item.item?.streamReady,
      )
      .map((item) => item.item);

    const estimatedBytes = tracks.reduce(
      (total, track) =>
        total + estimateBytesFromDuration(track.duration, quality),
      0,
    );
